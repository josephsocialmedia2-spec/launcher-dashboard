create or replace function public.f1_territory_sign_duplicates(
  ...
  p_comune text default '',
  p_via text default '',
  p_civico text default '',
  p_sign_type text default ''
)
returns jsonb
language plpgsql
security definer
...
declare
  v_uid uuid:=auth.uid();
  v_phone_digits text:=regexp_replace(coalesce(p_phone_normalized,''),'[^0-9]','','g');
begin
  if v_uid is null then raise exception 'ACCESSO RICHIESTO'; end if;
  if not exists(select 1 from public.f1_staff_profiles s where s.user_id=v_uid and s.status='ACTIVE') then
    raise exception 'PROFILO F1 NON AUTORIZZATO';
  end if;
  return coalesce((
    with d as (
      select
        'SIGN'::text source_type,o.observation_id::text record_id,o.observation_id,null::text lead_id,o.observed_at event_at,
        ...
        li.occurred_at last_call_at,li.outcome last_outcome,li.next_action last_next_action
      from public.f1_territory_observations o
      left join lateral (
        select i.occurred_at,i.outcome,i.next_action from public.interactions i
        where i.interaction_type='PHONE' and i.metadata->>'territory_observation_id'=o.observation_id::text
      ...
      union all
      select
        'LEAD'::text source_type,l.lead_id::text record_id,null::uuid observation_id,l.lead_id::text lead_id,l.updated_at event_at,
      ...
      left join lateral (
        select i.occurred_at,i.outcome,i.next_action from public.interactions i
        where i.interaction_type='PHONE' and i.lead_id=l.lead_id
        order by i.occurred_at desc limit 1
      ) li on true

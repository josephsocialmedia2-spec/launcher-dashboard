...
async function signInWithPassword(email,password){email=loginEmail(email);password=String(password||'');if(!email||!password)throw new Error('Inserisci email e password');const j=await authJson('token?grant_type=password',{method:'POST',body:JSON.stringify({email,password})});if(!j?.access_token||!j?.refresh_token)throw new Error('Sessione Supabase non ricevuta');return saveSession(j)}
async function signUpWithPassword(email,password,data={},redirectTo=''){email=loginEmail(email);password=String(password||'');if(!email||!password)throw new Error('Inserisci email e password');if(password.length<12)throw new Error('La password deve contenere almeno 12 caratteri');const qs=redirectTo?'?redirect_to='+encodeURIComponent(String(redirectTo)):'';const j=await authJson('signup'+qs,{method:'POST',body:JSON.stringify({email,password,data:data&&typeof data==='object'?data:{}})});if(j?.access_token&&j?.refresh_token)saveSession(j);return j}
async function sendMagicLink(email,redirectTo){email=loginEmail(email);if(!email)throw new Error('Inserisci la tua email');const qs=redirectTo?'?redirect_to='+encodeURIComponent(String(redirectTo)):'';await authJson('otp'+qs,{method:'POST',body:JSON.stringify({email,create_user:false,data:{}})});return true}
async function sendRecoveryEmail(email,redirectTo){email=loginEmail(email);if(!email)throw new Error('Inserisci la tua email');const qs=redirectTo?'?redirect_to='+encodeURIComponent(String(redirectTo)):'';await authJson('recover'+qs,{method:'POST',body:JSON.stringify({email})});return true}
...
async function currentUser(token){const res=await fetch(authBase()+'user',{headers:{apikey:CFG.anonKey,Authorization:'Bearer '+token}}),text=await res.text();let body=null;try{body=text?JSON.parse(text):null}catch(_){body=null}return{ok:res.ok,status:res.status,user:body}}

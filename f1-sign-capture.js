...
function openOverlay(options={}){
  ...
  if(!o){
    ...
    o.id='f1SignOverlay';
    o.className='f1-sign-overlay';
    o.innerHTML=`<section class="f1-sign-panel" role="dialog" aria-modal="true" aria-label="Leggi cartello"><div class="f1-sign-head"><div><div class="f1-sign-label">F1 TERRITORY · RACCOLTA</div><h2>📷 LEGGI CARTELLO</h2></div><button id="f1SignClose" class="f1-sign-close" type="button">×</button></div><div id="f1SignBody"></div></section>`;
    document.body.appendChild(o);
    $('f1SignClose').onclick=closeOverlay;
  }
  renderCapture();
  if(options?.autoTake){
...
async function getContext(){
  let st=null,p=null;
  if(navigator.onLine&&window.F1NotiziereEngine?.load){
    try{
      st=await F1NotiziereEngine.load({force:true});
      p=st?.territory?.progress||null;
      if(st)await window.F1MobileStore?.cacheState?.(st);
    }catch(e){console.warn('[F1 sign context online]',e)}
  }
  if(!p){
    try{
      const cached=await window.F1MobileStore?.cachedState?.();
      const cp=cached?.territory?.progress;
      if(cp?.progress_id&&txt(cp.comune)&&txt(cp.via)){st=cached;p=cp}
    }catch(e){console.warn('[F1 sign cached context]',e)}
  }
  if(!p&&navigator.onLine&&window.F1StaffData?.ready?.()){
    try{
      const crm=await F1StaffData.rpc('f1_territory_mobile_crm_v5',{p_limit:700}),last=crm?.last_position||{};
      if(txt(last.comune)&&txt(last.via)){
        const opened=await F1StaffData.rpc('f1_territory_open_street_v3',{p_comune:txt(last.comune),p_via:txt(last.via),p_source:'HOME CARTELLO',p_source_ref:txt(last.civic_record_id||'')});
        if(opened?.progress_id&&txt(last.civico))await F1StaffData.rpc('f1_territory_set_manual_civic_v3',{p_progress_id:opened.progress_id,p_civico:txt(last.civico)});
        window.F1NotiziereEngine?.invalidate?.();
        st=await F1NotiziereEngine.load({force:true});
        p=st?.territory?.progress||opened||null;
        if(st)await window.F1MobileStore?.cacheState?.(st);
      }
    }catch(e){console.warn('[F1 sign restore last position]',e)}
  }
  if(!p?.progress_id||!txt(p.comune)||!txt(p.via))throw new Error('SELEZIONA PRIMA COMUNE / VIA / CIVICO');
  const uiCivic=txt(document.getElementById('civicInput')?.value),civico=uiCivic||txt(p.next_civic||p.civic_start||p.last_civic);
  if(!civico)throw new Error('SELEZIONA PRIMA COMUNE / VIA / CIVICO');
...
async function geolocateAndFill(){
  ...
  try{
    ...
    if($('f1LocComune')&&comune)$('f1LocComune').value=comune;
    if($('f1LocVia')&&via)$('f1LocVia').value=via;
    if($('f1LocCivico')&&civico)$('f1LocCivico').value=civico;
    if($('f1LocZona')&&zona)$('f1LocZona').value=zona;
...
function nextUsefulDate(){const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Rome',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()).reduce((a,p)=>(a[p.type]=p.value,a),{});let d=new Date(Date.UTC(Number(parts.year),Number(parts.month)-1,Number(parts.day),12));do{d.setUTCDate(d.getUTCDate()+1)}while(d.getUTCDay()===0);return d.toISOString().slice(0,10)}
...
async function ensureCivicRecordForSign(payload){
  ...
  try{return await F1StaffData.rpc('f1_territory_ensure_civic_v2',{p_progress_id:payload.progress_id,p_civico:d.civico})}catch(first){
    let restore='';try{const st=await F1NotiziereEngine.load({force:true}),p=st?.territory?.progress;if(p?.progress_id===payload.progress_id)restore=txt(p.next_civic||p.civic_start);await F1StaffData.rpc('f1_territory_set_manual_civic_v3',{p_progress_id:payload.progress_id,p_civico:d.civico});const id=await F1StaffData.rpc('f1_territory_ensure_civic_v2',{p_progress_id:payload.progress_id,p_civico:d.civico});if(restore&&restore!==d.civico)await F1StaffData.rpc('f1_territory_set_manual_civic_v3',{p_progress_id:payload.progress_id,p_civico:restore}).catch(()=>{});return id}catch(_){throw first}
  }
}
async function saveSignCrmNote(payload,row,photoPath){
  const civicId=await ensureCivicRecordForSign(payload),d=payload.data,when=new Intl.DateTimeFormat('it-IT',{timeZone:'Europe/Rome',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(payload.captured_at)),note='CARTELLO · '+when+'\n\nComune: '+d.comune+'\nVia: '+d.via+'\nCivico: '+d.civico+'\n\nTESTO ACQUISITO:\n'+d.ocr_confirmed+'\n\nTELEFONO RILEVATO:\n'+(d.phone||'NESSUN TELEFONO RILEVATO')+'\n\nTIPO:\n'+d.sign_type+(d.property_type?'\nTIPO IMMOBILE: '+d.property_type:'')+'\nAGENZIA RILEVATA: '+(/AGENZIA|IMMOBILIARE/i.test(d.ocr_confirmed)?'SÌ':'NO')+'\n\nFOTO:\n'+(photoPath||'NON DISPONIBILE')+'\n\nORIGINE:\nCARTELLO TERRITORIALE\n\nOSSERVAZIONE ID: '+row.observation_id;
  const crm=await F1StaffData.rpc('f1_territory_mobile_crm_v5',{p_limit:2000}).catch(()=>null),exists=crm?.notes?.some(n=>n.civic_record_id===civicId&&String(n.note_text||'').includes('OSSERVAZIONE ID: '+row.observation_id));
  if(!exists)await F1StaffData.rpc('f1_territory_note_add_v4',{p_civic_record_id:civicId,p_note_type:'OCR',p_note_text:note,p_audio_path:'',p_audio_mime:'',p_audio_duration_seconds:null,p_photo_path:photoPath||'',p_source_file_name:'cartello.jpg'});
  return civicId;
}

...
test('avvio, asset interni e console senza errori critici', async ({ page, request }) => {
  ...
  for (const path of [
    APP,
    '/acquisitore-pro/app-v24.js',
    '/acquisitore-pro/app-v24-extra.js',
    '/acquisitore-pro/manifest-v24.webmanifest',
    '/acquisitore-pro/sw-v24.js',
    '/acquisitore-pro/icon-192.svg',
    '/acquisitore-pro/icon-512.svg'
  ]) {
...
test('SONO QUI: permesso geolocalizzazione, reverse geocoding e fallback UI', async ({ page, context }) => {
  ...
  await page.route('https://nominatim.openstreetmap.org/**', async route => {
    await route.fulfill({
      ...
      body: JSON.stringify({ address: { town: 'Avigliana', road: 'Via Roma', house_number: '10' } })
    });
  });
  await openApp(page);

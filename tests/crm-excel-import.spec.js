...
test('XLSX XLS CSV -> preview -> dedupe -> unified CRM',async({page})=>{
  ...
  await page.route('https://nqnmlsmeiynxbdojeyjt.supabase.co/auth/v1/user',async r=>{expect(r.request().headers().authorization||'').toContain('Bearer qa');await r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({id:'00000000-0000-4000-8000-000000000001',role:'authenticated'})})});

...
const $=id=>document.getElementById(id),PROGRAM='F1_90_DAY_PROGRAM',DAY='F1_90_DAY_DAY';
...
async function boot(){try{
  ...
  const today=F1AcquisitionCore.todayRome(),[campaigns,events,interactions,tasks]=await Promise.all([
    F1AcquisitionData.rest('campaigns?select=*&order=created_at.desc&limit=1000'),F1AcquisitionData.rest('events?select=*&order=occurred_at.desc&limit=3000'),F1AcquisitionData.pullInteractions(),F1AcquisitionData.pullTasks()

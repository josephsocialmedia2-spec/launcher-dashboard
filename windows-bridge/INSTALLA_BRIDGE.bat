...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ws=New-Object -ComObject WScript.Shell; $desk=[Environment]::GetFolderPath('Desktop'); $s=$ws.CreateShortcut((Join-Path $desk 'F1 - Configura Sync e Ollama.lnk')); $s.TargetPath='%ROOT%\CONFIGURA_BRIDGE.cmd'; $s.WorkingDirectory='%ROOT%'; $s.Save(); $g=$ws.CreateShortcut((Join-Path $desk 'F1 - ChatGPT Login.lnk')); $g.TargetPath='%ROOT%\CHATGPT_LOGIN.cmd'; $g.WorkingDirectory='%ROOT%'; $g.Save(); $startup=[Environment]::GetFolderPath('Startup'); $a=$ws.CreateShortcut((Join-Path $startup 'F1 Bridge Contatti.lnk')); $a.TargetPath='%ROOT%\AVVIA_BRIDGE.cmd'; $a.WorkingDirectory='%ROOT%'; $a.Save()"

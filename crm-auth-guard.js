...
function pReport(){const names=['AUTH','COUNT_KPI','LOAD_FIRST_50','LOAD_VISIBLE_TASKS','FIRST_RENDER','TIME TO USABLE','DETAIL_LOAD','INTERACTIONS_LOAD','DOSSIER_LOAD'];const rows=names.map(fase=>{const ms=pDuration(fase);return{FASE:fase,MS:ms==null?'NON MISURATO':Math.round(ms*10)/10}});try{console.table(rows)}catch(_){console.log('F1 PERFORMANCE',rows)}return rows}
pMark('F1_BOOT_START');
...
async function ensure(){const ok=await validation;if(!ok){showGate();throw authError()}return true}
window.F1CRMPerf={mark:pMark,measure:pMeasure,duration:pDuration,report:pReport,marks:perfMarks};
window.F1CRMAuthGuard={ready:()=>authenticated,validated:()=>resolved,ensure,show:showGate,loginUrl:LOGIN};

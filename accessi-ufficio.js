...
function setMsg(text,bad=false){const e=$('msg');e.textContent=text||'';e.className='msg'+(text?' show '+(bad?'bad':'ok'):'')}
...
function renderAccounts(){
  ...
  const box=$('accounts');
  box.innerHTML=state.accounts.length?state.accounts.map((a,i)=>{const status=statusOf(a),email=txt(a.email||a.company_email),name=[a.first_name,a.last_name].filter(Boolean).join(' ')||email||'Account F1';return `<article class="account" data-index="${i}"><div class="account-head"><div><strong>${esc(name)}</strong><small>${esc(a.role||'—')} · ${esc(email||'—')}</small></div><span class="badge ${status==='ACTIVE'?'':'off'}">${esc(status)}</span></div><div class="meta"><span>ULTIMO LOGIN</span><strong>${esc(fmtDate(a.last_sign_in_at))}</strong><span>ULTIMO ACCESSO</span><strong>${esc(fmtDate(a.last_access))}</strong><span>ACCESSI</span><strong>${Number(a.access_count||0)}</strong><span>PASSWORD</span><strong>PROTETTA · NON VISUALIZZABILE</strong></div><div class="actions">${email?'<button class="btn secondary" data-copy>COPIA EMAIL</button><button class="btn dark" data-recovery>INVIA RECUPERO</button>':''}${status==='ACTIVE'?'<button class="btn danger" data-toggle>DISATTIVA</button>':'<button class="btn primary" data-toggle>RIATTIVA</button>'}</div><div class="password-box"><div class="password-row"><input type="password" data-new-password minlength="12" autocomplete="new-password" placeholder="Nuova password temporanea"><button class="btn secondary" data-generate>GENERA</button><button class="btn dark" data-set-password>IMPOSTA PASSWORD</button></div></div></article>`}).join(''):'<div class="empty">NESSUN ACCOUNT REGISTRATO</div>';
...
async function init(){
  try{
    ...
    if(String(state.me.role||'').toUpperCase()!=='TITOLARE'){document.body.innerHTML='<main class="wrap"><section class="card"><h1>ACCESSO RISERVATO AL TITOLARE</h1><a class="btn dark" href="ricerca-territoriale.html">TORNA ALLA DASHBOARD</a></section></main>';reveal();return}

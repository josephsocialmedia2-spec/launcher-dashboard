...
class SadTalkerProvider:
    ...
    def animate(self,image:Path,audio:Path,result_dir:Path)->Path:
        ...
        cmd=[str(self.python_exe),"inference.py","--driven_audio",str(audio.resolve()),"--source_image",str(image.resolve()),"--result_dir",str(result_dir.resolve()),"--preprocess","full","--still"]
